...
gh issue create \
  --repo "$REPO" \
  --title "[PRIORITY] Create printable QR code labels feature" \
...
gh issue create \
  --repo "$REPO" \
  --title "[PRIORITY] Simplify QR codes to alphanumeric format" \
...
gh issue create \
  --repo "$REPO" \
  --title "[PRIORITY] Add 'Add Item + Create More' button" \
...
gh issue create \
  --repo "$REPO" \
  --title "[PRIORITY] Create bulk entry mode" \
...
gh issue create \
  --repo "$REPO" \
  --title "[PRIORITY] Implement database backup/restore functionality" \
...
gh issue create \
  --repo "$REPO" \
  --title "[PRIORITY] Use USDA/FDA recommendations for category expiration dates" \
...
gh issue create \
  --repo "$REPO" \
  --title "[PRIORITY] Add stock images based on item type" \
...
gh issue create \
  --repo "$REPO" \
  --title "[HIGH] Implement barcode scanning support" \
...
gh issue create \
  --repo "$REPO" \
  --title "[MEDIUM] Add bulk import/export functionality (CSV, JSON)" \
...
gh issue create \
  --repo "$REPO" \
  --title "[MEDIUM] Add PostgreSQL migration option" \
...
gh issue create \
  --repo "$REPO" \
  --title "[MEDIUM] Create Raspberry Pi deployment guide" \
...
gh issue create \
  --repo "$REPO" \
  --title "[MEDIUM] Create AWS/GCP free tier deployment guide" \
...
gh issue create \
  --repo "$REPO" \
  --title "[MEDIUM] Add production WSGI server configuration" \
...
gh issue create \
  --repo "$REPO" \
  --title "[LOW] Create email notifications for items expiring soon" \
...
gh issue create \
  --repo "$REPO" \
  --title "[LOW] Add inventory statistics dashboard" \
...
gh issue create \
  --repo "$REPO" \
  --title "[MOBILE] Add QR code scanning with device camera" \
...
gh issue create \
  --repo "$REPO" \
  --title "[MOBILE] Implement mobile-responsive design" \
